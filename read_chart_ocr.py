...
class ReadChartImage:
    ...
    def auto_resize_image(self):
        scalar_options = [int(7000/self.img_height), int(6000/self.img_width)]
    ...
    def merge_lines(self, vert_tolerance=100, horizontal_tolerance=80, overlap_tolerance=50) -> None:
        if self.lines is not None:
            for i in range(0, len(self.lines)):
                ln = self.lines[i][0]
    ...
    def create_merge_lines(self, vert_tolerance=100, horizontal_tolerance=80, overlap_tolerance=50) -> None:
        self.create_lines()
    ...
    @staticmethod
    def read_box(img, tesseract_path: str) -> str:
    ...
    def read_chart(self) -> list:
        ...
        for i in range(len(self.horizontal_lines) - 1):
            ...
            if len(self.vertical_lines) == 0:
                ...
                result = self.read_box(cropped_img, tesseract_path=self.tesseract_path)
                lst.append(result)

            elif len(self.vertical_lines) >= 1:
